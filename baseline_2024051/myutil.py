...
class Rider:
    ...
    def calculate_cost(self, dist):
        cost = int(self.fixed_cost) + int(dist / 100.0 * self.var_cost)
        print(cost)
        return cost
...
def try_merging_bundles_with_rider(all_orders, bundle1, bundle2, rider):
...
# Note: 이 때 배달원은 두 개의 주어진 bundle을 배달하는 배달원들만 후보로 테스트(주어진 bundle에 사용되지 않는 배달원을 묶는게 가능할 수 있음!)
# Note: 여러개의 배달원으로 묶는게 가능할 때 가장 먼저 가능한 배달원 기준으로 반환(비용을 고려하지 않음)
def try_single_bundle_with_rider(all_orders, bundle1, rider):
    merged_orders = bundle1.shop_seq
    total_volume = get_total_volume(all_orders, merged_orders)
